...
namespace Destiny2.Common.Weapons
{
    ...
    public abstract partial class Destiny2WeaponItem : ModItem
    {
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            ...
            if (magazineSize > 0)
            {
                ...
                {
                    // Touch of Malice: Infinite Ammo + Health Drain
                    int drain = (int)(player.statLifeMax2 * TouchOfMalicePerk.HealthDrainPercent);
                    if (player.statLife > 1)
                    {
                        int actualDrain = Math.Min(drain, player.statLife - 1);
                        if (actualDrain > 0)
                        {
                            player.statLife -= actualDrain;
                            CombatText.NewText(player.getRect(), Color.Red, actualDrain);
            ...
            if (type == bulletType && HasPerk<ExplosiveShadowPerk>())
                type = slugType;
        ...
        internal bool HasPerk<TPerk>() where TPerk : Destiny2Perk
        {
            foreach (Destiny2Perk perk in GetPerks())
            {
                if (perk is TPerk)
